...
namespace YimMenu::Submenus
{
	    ...
	    Submenu::Submenu("Mundo", ICON_FA_GLOBE)
	{
		...
		auto deleteOpts = std::make_shared<Group>("Borrar", 2);
		...
		auto bringOpts = std::make_shared<Group>("Traer", 1);
		...
		auto weatherOpts = std::make_shared<Group>("Clima", 1);
		...
		auto timeGroup = std::make_shared<Group>("Hora");

		auto hms = std::make_shared<Group>("", 1);
		hms->AddItem(std::make_shared<IntCommandItem>("networktimehour"_J, "Hora"));
		hms->AddItem(std::make_shared<IntCommandItem>("networktimeminute"_J, "Minuto"));
		hms->AddItem(std::make_shared<IntCommandItem>("networktimesecond"_J, "Segundo"));
		timeGroup->AddItem(std::move(hms));

		timeGroup->AddItem(std::make_shared<CommandItem>("setnetworktime"_J, "Establecer"));
		timeGroup->AddItem(std::make_shared<BoolCommandItem>("freezenetworktime"_J, "Congelar"));

		auto otherOpts = std::make_shared<Group>("Otros", 1);
