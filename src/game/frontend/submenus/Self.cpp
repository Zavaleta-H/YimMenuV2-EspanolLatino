...
namespace YimMenu::Submenus
{
	Self::Self() :
	    ...
	    Submenu::Submenu("Personaje", ICON_FA_USER)
	{
		auto main = std::make_shared<Category>("Principal");
		auto globalsGroup = std::make_shared<Group>("Globales");
		auto movementGroup = std::make_shared<Group>("Movimiento");
		auto toolsGroup = std::make_shared<Group>("Herramientas", 2);
		auto specialAbilityGroup = std::make_shared<Group>("Habilidad especial");
		auto wantedGroup = std::make_shared<Group>("Búsqueda");
		...
		setWanted->AddItem(std::make_shared<IntCommandItem>("wantedslider"_J, "Nivel"));
		...
		specialInMp->AddItem(std::make_shared<BoolCommandItem>("mpspecialability"_J, "Activa en MP"));
