...
namespace YimMenu::Submenus
{
	Vehicle::Vehicle() :
	    ...
	    Submenu::Submenu("Vehículo", ICON_FA_CAR)
	{
		auto main = std::make_shared<Category>("Principal");

		auto globals = std::make_shared<Group>("Globales");
		auto tools = std::make_shared<Group>("Herramientas", 2);
		auto misc = std::make_shared<Group>("Otros");

		globals->AddItem(std::make_shared<BoolCommandItem>("vehiclegodmode"_J, "Modo dios"));
		globals->AddItem(std::make_shared<BoolCommandItem>("keepfixed"_J, "Mantener arreglado"));
		...
		misc->AddItem(std::make_shared<BoolCommandItem>("lowervehiclestance"_J, "Bajar postura"));
